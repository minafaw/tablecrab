...
class FrameSettings(QtGui.QFrame):
	...
	SettingsKeyBase = 'Gui/Settings/ICMTax/StyleSheet'
	...
	def onInitSettings(self):
		...
		if not value:
			value = Tc2GuiHandViewer.BrowserSideBarICMTax.StyleSheet
		self.edit.setPlainText(value)
		self.edit.textChanged.connect(
				lambda self=self: self.setStyleSheet(self.edit.toPlainText())
				)

		Tc2Config.globalObject.objectCreatedSettingsICMTaxStyleSheet.emit(self)
	...
	def onHelp(self, *args):
		Tc2GuiHelp.dialogHelp('settingsICMTaxStyleSheet', parent=self)

	#TODO: resetting document jumps to top of widget. store/restore position would be nice
	def onRestoreDefault(self):
		self.edit.setPlainText(Tc2GuiHandViewer.BrowserSideBarICMTax.StyleSheet)
