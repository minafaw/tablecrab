...
class FrameSettings(QtGui.QFrame):
	...
	def onEditTextChanged(self):
		...
		Tc2Config.settingsSetValue(Tc2GuiToolsHandHistoryViewer.FrameNashCalculations.SettingsKeyStyleSheet, text)

	def onInit(self):
		self.layout()

		#NOTE: style sheet can not be ''
		text = Tc2Config.settingsValue(Tc2GuiToolsHandHistoryViewer.FrameNashCalculations.SettingsKeyStyleSheet, '').toString()
		if not text:
